...
import { useInstanceService, useRepositoryService } from "@/context/service_context";
...
function InstancesPage() {
  ...
  const [status, setStatus] = useState<RequestStatus<{ repositories: Repository[]; instances: Instance[] }>>({
    status: "idle",
  });
        ...
        return (
          <div key={repo.id} className="border p-3 rounded-lg">
            ...
            {repoInstances.length === 0 ? (
              <p className="text-gray-500">No instance associate to this repository.</p>
            ) : (
              <ul className="space-y-2">
                {repoInstances.map((inst) => (
                  <li key={inst.id} className="border p-2 rounded-md">
                    ...
                    {inst.occupiedBy && (
                      <div className="text-sm text-gray-600">Occupied By: {inst.occupiedBy}</div>
                    )}
                  </li>
                ))}
              </ul>
